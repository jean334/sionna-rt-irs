...
scene.render_to_file(camera=bird_cam,
                        fov=90,
                        filename=os.path.join(save_dir, f"scene_bird_cam.png")
                        )
...
"""
sources = [[-8],[1],[0]]
targets = [ [[-4],[1],[i/2]] for i in range(-16, 16, 1)]
for i,target in enumerate(targets):
    ris.phase_gradient_reflector(sources, target)
    ris.amplitude_profile.show(path=os.path.join(save_dir, "gif"), i=i)
    ris.phase_profile.show(path=os.path.join(save_dir, "gif"), i=i)
    rm = solver(scene, 
                center= cm_center,
    ...
    scene.render_to_file(camera=bird_cam,
                            ...
                            rm_vmax=0,
                            rm_vmin=-60,
                            filename=os.path.join(save_dir, "gif", f"scene_rm_bird_cam_{i}.png"))
    ...
    scene.render_to_file(camera=cam2,
                            ...
                            rm_vmax=0,
                            rm_vmin=-60,
                            filename=os.path.join(save_dir, "gif", f"scene_rm_cam2_{i}.png"))
