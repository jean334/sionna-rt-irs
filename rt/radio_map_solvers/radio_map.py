...
from sionna.rt.constants import DEFAULT_TRANSMITTER_COLOR,\
    DEFAULT_RECEIVER_COLOR, DEFAULT_IRS_COLOR


from rt.utils.legacy_utils import compute_spreading_factor,safe_div
...
class RadioMap:
    ...
    def __init__(self,
        ...
        irs_positions_x = mi.Float([irs.position.x[0]
                                    for irs in scene.irs.values()])
        irs_positions_y = mi.Float([irs.position.y[0]
                                    for irs in scene.irs.values()])
        irs_positions_z = mi.Float([irs.position.z[0]
                                    for irs in scene.irs.values()])
        self._irs_positions = mi.Point3f(irs_positions_x,
                                          irs_positions_y,
                                          irs_positions_z)

        # Builds the Mitsuba rectangle modeling the measurement plane
        meas_plane = mi.load_dict({'type': 'rectangle'})
    ...
    def num_irs(self):
        r"""Number of IRS"""
        return dr.width(self._irs_positions)
    ...
    @property
    def irs_cell_indices(self):
        r"""Computes and returns the cell index positions corresponding to
        IRS in the format `(column, row)`

        :type: :py:class:`mi.Point2u`
        """
        return self._global_to_cell_ind(self._irs_positions)
    ...
    def add(
        self,
        e_fields : List[mi.Vector4f],   # assuming a list of vectors
        solid_angle : mi.Float,
        array_w : List,                 # whatever type does `@` with e_field
        si_mp : mi.SurfaceInteraction3f,
        k_world : mi.Vector3f,
        ray_origin: mi.Vector3f,
        tx_indices : mi.UInt,
        hit : mi.Bool,
        ris: bool,
        radii_curv: mi.Vector2f,
    ) -> None:
        # ---- Avoid compaction; keep everything full-width and mask ----

        # Safe UVs for non-hit lanes (avoid NaNs/undefined indices)
        #uv_safe = dr.select(hit, si_mp.uv, mi.Point2f(0.0))  # or mi.Vector2f(0)

        # Indices of the hit cells (computed from safe UVs)
        cell_ind = self._local_to_cell_ind(si_mp.uv)#uv_safe)

        # Indices into the radio-map tensor
        num_cells = self._num_cells.x[0] * self._num_cells.y[0]
        tensor_ind = tx_indices * num_cells + cell_ind

        # Optional RIS pre-factor (only affect active lanes)
        
        if ris:
            length = dr.norm(si_mp.p - ray_origin)
            sf = compute_spreading_factor(radii_curv.x, radii_curv.y, length)
            sf = dr.select(hit, sf, mi.Float(1.0))
            #sf = 1
            # scale fields only; keep width unchanged
            e_fields_scaled = [ef * sf for ef in e_fields]
        else:
            e_fields_scaled = e_fields
        
        # Contribution to the path loss map
        a = dr.zeros(mi.Vector4f)   # full-width zero
        for e_field, aw in zip(e_fields_scaled, array_w):
            a += aw @ e_field
        a = dr.squared_norm(a)  # per-lane squared norm

        # Ray weight from projected solid angle
        #safe_div(solid_angle, cos_theta)  # solid_angle * rcp(cos_theta)

        # Additional RIS weights (no compaction; mask instead)
        k_local = si_mp.to_local(k_world)
        cos_theta = dr.abs(k_local.z)
        #ris = False
        if ris:
            # Advance radii only on hit lanes
            new_radii = dr.select(hit, radii_curv + length, radii_curv)
            num = new_radii.x * new_radii.y
            w = num* dr.rcp(cos_theta) * solid_angle
            #with open("results/num.txt", "w") as f:
            #    for elem in num:
            #        f.write(f"{elem}\n")
            a *= w
        else:            
        ...
        # Scatter only where hit is True (mask applies at the write)
        dr.scatter_reduce(
            dr.ReduceOp.Add,
    ...
    def show(
        ...
        irs_cell_indices = self.irs_cell_indices
        ...
        if show_irs:
            for irs in range(self.num_irs):
                fig_cm.axes[0].scatter(irs_cell_indices.x[irs],
                                       irs_cell_indices.y[irs],
                                       marker='*',
                                       color=DEFAULT_IRS_COLOR)
