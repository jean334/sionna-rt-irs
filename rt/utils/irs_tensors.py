...
def expand_to_rank_dr(tensor, target_rank, axis=-1):
    ...
    """
    num_dims = dr.max(target_rank - len(tensor.shape), axis = 0)
    output = insert_dims_dr(tensor, num_dims, axis)
...
def insert_dims_dr(tensor, num_dims, axis=-1):
    ...
    new_shape.extend([1]*num_dims)
    new_shape.extend(shape[axis:])
    output = dr.reshape(dr.cuda.TensorXf, tensor, new_shape)
